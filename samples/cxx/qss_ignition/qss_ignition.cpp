...
void runexample() {
  ...
  // import kinetics
  cout << "import kinetics..." << endl;
  shared_ptr<Kinetics> kin_qss(newKineticsMgr(
      gas_qss.xml(), vector<ThermoPhase *>{&gas_bulk, &gas_qss}));
  ...
  cout << "create a reactor net..." << endl;
  ReactorNet sim;
  sim.addReactor(r);

  // advance reactor till T >= T0 + 500 K
  cout << "advance reactor..." << endl;
  const double dt = 1.0e-7;
  double tm = 0.0;
  while (true) {
    sim.advance(tm);
    tm += dt;
    if (gas_bulk.temperature() >= 2000.0)
      break;
  }

  ofstream f("qss_ignition_cxx.csv");
  vector<size_t> k_out{
      gas_bulk.speciesIndex("CH3OCH3"), gas_bulk.speciesIndex("O2"),
      gas_bulk.speciesIndex("CO2"),     gas_bulk.speciesIndex("H2O"),
      gas_bulk.speciesIndex("CO"),      gas_bulk.speciesIndex("OH"),
      gas_bulk.speciesIndex("H")};
  f << tm << ", " << gas_bulk.temperature() << ", ";
