...
public:
  ...
  virtual std::string kineticsType() const { return "Kinetics"; }

  //! Number of reactions in the reaction mechanism.
  size_t nReactions() const { return m_reactions.size(); }
   ...
   */
  size_t nTotalSpecies() const { return m_kk; }
   ...
   * @param k          Species index
   */
  thermo_t &speciesPhase(size_t k) { return thermo(speciesPhaseIndex(k)); }
  ...
  void skipUndeclaredSpecies(bool skip) { m_skipUndeclaredSpecies = skip; }
   ...
   */
  virtual void setMultiplier(size_t i, doublereal f) { m_perturb[i] = f; }
  ...
  checkDuplicates(bool throw_err = true) const;
...
protected:
  ...
  virtual void updateROP() { throw NotImplementedError("Kinetics::updateROP"); }
