...
public:
  ...
  // get m_iactiv
  const std::vector<std::uint8_t> &getActiv() const { return m_iactiv; }

  // get numer of activ reactions
  size_t getNumActiv() const { return m_nactiv; }
...
protected:
  ...
  //! Calculate m_nactiv
  size_t updateNumActiv(const std::vector<std::uint8_t> &_iactiv);
};
}
