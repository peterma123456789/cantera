...
const int c_Soret = 2; // this seems useless
const int c_Trans_Transport = 15;
...
class StFlow : public Domain1D
{
  ...
  public:
    ...
    void enableUnityLe(bool withUnityLe);
  ...
  protected:
    ...
    doublereal dhbardz(const doublereal* x, size_t j, size_t k)
    {
        size_t jloc = (u(x, j) > 0.0 ? j : j + 1);
...
//        doublereal h1 = hbar[k];
//
//        setGas(x, jloc - 1);
//        m_thermo->getPartialMolarEnthalpies(&hbar[0]);
//        doublereal h2 = hbar[k];
//
//        // return back to the previous state
//        setGas(x, j);
//
//        return (h1 - h2) / m_dz[jloc - 1];

        return (m_hbar[jloc * m_nsp + k] - m_hbar[(jloc - 1) * m_nsp + k]) / m_dz[jloc - 1];
    }
...
class AxiStagnFlow : public StFlow
{
  public:
    ...
    //AxiStagnFlow(IdealGasPhase* ph = 0, size_t nsp = 1, size_t points = 1) : StFlow(ph, nsp, points)
    //AxiStagnFlow(PengRobinsonGasPhase* ph = 0, size_t nsp = 1, size_t points = 1) : StFlow(ph, nsp, points)
    AxiStagnFlow(ThermoPhase* ph = 0, size_t nsp = 1, size_t points = 1) : StFlow(ph, nsp, points)
    {
        m_dovisc = true;
    }
