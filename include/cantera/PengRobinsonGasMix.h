...
class PengRobinsonGasMix : public PengRobinsonGasPhase, public GasKinetics
{
  public:
    ...
    PengRobinsonGasMix(const std::string& infile, std::string id_ = "") : m_ok(false), m_r(0)
    {
        ...
        if (!m_ok) throw CanteraError("PengRobinsonGasMix",
                                      "Cantera::buildSolutionFromXML returned false");
    }

    PengRobinsonGasMix(XML_Node& root, std::string id_) : m_ok(false), m_r(&root), m_id(id_)
    {
        m_ok = buildSolutionFromXML(root, id_, "phase", this, this);
    }

    PengRobinsonGasMix(const PengRobinsonGasMix& other) : m_ok(false), m_r(other.m_r), m_id(other.m_id)
    {
        m_ok = buildSolutionFromXML(*m_r, m_id, "phase", this, this);
    }
