...
//StFlow::StFlow(IdealGasPhase* ph, size_t nsp, size_t points) : Domain1D(nsp + 4, points),
//                                                               m_press(-1.0),
//                                                               m_nsp(nsp),
//                                                               m_thermo(0),
//                                                               m_kin(0),
...
//StFlow::StFlow(PengRobinsonGasPhase* ph, size_t nsp, size_t points) : Domain1D(nsp + 4, points),
//                                                                      m_press(-1.0),
//                                                                      m_nsp(nsp),
//                                                                      m_thermo(0),
//                                                                      m_kin(0),
...
//                                                                      m_transport_option(-1),
//                                                                      m_do_radiation(false)
StFlow::StFlow(ThermoPhase* ph, size_t nsp, size_t points) : Domain1D(nsp + 4, points),
                                                                      m_press(-1.0),
                                                                      m_nsp(nsp),
                                                                      m_thermo(0),
                                                                      m_kin(0),
                                                                      ...
                                                                      m_do_radiation(false)
{
    ...
    // Peter Modified
    setBounds(2, 1.0, 1e9); // temperature bounds
    setBounds(3, -1e40, 1e40); // lambda should be negative
...
void StFlow::setTransport(Transport& trans, bool withSoret)
{
    ...
    if (model == cMulticomponent || model == CK_Multicomponent) {
        ...
        m_diff.resize(m_nsp * m_points);
        m_dthermal.resize(m_nsp, m_points, 0.0);
    } else if (model == cTrans_LP || model == cTrans_HP || model == cBlend) {
        m_transport_option = c_Trans_Transport;
        m_multidiff.resize(m_nsp * m_nsp * m_points);
        m_diff.resize(m_nsp * m_points);
        m_dthermal.resize(m_nsp, m_points, 0.0);
...
{
    if (m_transport_option == c_Multi_Transport || m_transport_option == c_Trans_Transport) {
        m_do_soret = withSoret;
    } else {
        throw CanteraError("setTransport",
                           "Thermal diffusion (the Soret effect) "
...
void StFlow::enableUnityLe(bool withUnityLe)
{
    if (m_transport_option == c_Mixav_Transport || m_transport_option == CK_MixtureAveraged) {
        m_do_unityLe = withUnityLe;
    } else {
        throw CanteraError("setTransport",
                           "Unity Le number "
                           "requires using a mixture averaged transport model.");
    }
}
                  ...
                  doublereal* rg, integer* diagg, doublereal rdt)
{
    ...
    if (m_do_radiation) {
        ...
        for (size_t j = jmin; j < jmax; j++) {
            ...
            radiative_heat_loss = 2 * k_P * (2 * StefanBoltz * pow(T(x, j), 4) - boundary_Rad_left - boundary_Rad_right);
    ...
    for (j = jmin; j <= jmax; j++) {
        ...
        if (j == 0) {
            ...
            rsd[index(c_offset_U, 0)] =
                -(rho_u(x, 1) - rho_u(x, 0)) / m_dz[0] - (density(1) * V(x, 1) + density(0) * V(x, 0));
            ...
            rsd[index(c_offset_V, j)] =
                (shear(x, j) - lambda(x, j) - rho_u(x, j) * dVdz(x, j) - m_rho[j] * V(x, j) * V(x, j)) / m_rho[j] - rdt * (V(x, j) - V_prev(j));
            ...
            for (k = 0; k < m_nsp; k++) {
                ...
                rsd[index(c_offset_Y + k, j)] =
                    (m_wt[k] * wdot(k, j) - convec - diffus) / m_rho[j] - rdt * (Y(x, k, j) - Y_prev(k, j));
            ...
            if (m_do_energy[j]) {
                ...
                //setGas(x, j); // do we need this???

                // heat release term
                //const vector_fp& h_RT = m_thermo->enthalpy_RT_ref();
                //const vector_fp& cp_R = m_thermo->cp_R_ref();
                //vector_fp hbar(m_nsp);
                //m_thermo->getPartialMolarEnthalpies(&hbar[0]);
                ...
                rsd[index(c_offset_T, j)] =
                    -m_cp[j] * rho_u(x, j) * dtdzj - divHeatFlux(x, j) - sum - sum2;
...
{
//    if (m_transport_option == c_Mixav_Transport || m_transport_option == c_Trans_Transport) {
    if (m_transport_option == c_Mixav_Transport) {
    ...
    } else if (m_transport_option == c_Multi_Transport || m_transport_option == c_Trans_Transport) {
        ...
        case c_Trans_Transport:
            for (j = j0; j < j1; j++) {
                ...
                for (k = 0; k < m_nsp; k++) {
                    ...
                    //if (k == 4 && m_thermo->temperature() < 300.0) sum *= 0.9999999;
                                  ...
                                  integer* diag, doublereal rdt)
{
    ...
    rsd[index(c_offset_U, j)] =
        -(rho_u(x, j + 1) - rho_u(x, j)) / m_dz[j] - (density(j + 1) * V(x, j + 1) + density(j) * V(x, j));

    //algebraic constraint
    diag[index(c_offset_U, j)] = 0;
}

//FreeFlame::FreeFlame(IdealGasPhase* ph, size_t nsp, size_t points) : StFlow(ph, nsp, points),
//                                                                     m_zfixed(Undef),
//                                                                     m_tfixed(Undef)
//FreeFlame::FreeFlame(PengRobinsonGasPhase* ph, size_t nsp, size_t points) : StFlow(ph, nsp, points),
//                                                                            m_zfixed(Undef),
//                                                                            m_tfixed(Undef)
FreeFlame::FreeFlame(ThermoPhase* ph, size_t nsp, size_t points) : StFlow(ph, nsp, points),
                                                                            m_zfixed(Undef),
                                                                            m_tfixed(Undef)
{
    m_dovisc = false;
                               ...
                               integer* diag, doublereal rdt)
{
    ...
    if (grid(j) > m_zfixed) {
        rsd[index(c_offset_U, j)] =
            -(rho_u(x, j) - rho_u(x, j - 1)) / m_dz[j - 1] - (density(j - 1) * V(x, j - 1) + density(j) * V(x, j));
    }
