...
MultiNewton::MultiNewton(int sz) : m_maxAge(5)
{
    m_n = sz;
    m_elapsed = 0.0;
}
...
#undef DEBUG_STEP
    ...
    if (iok != 0) {
        ...
        throw CanteraError("MultiNewton::step",
                           "Jacobian is singular for domain " +
                               dom.id() + ", component " + dom.componentName(comp) + " at point " + int2str(pt) + "\n(Matrix row " + int2str(iok) + ") \nsee file bandmatrix.csv\n");
    } else if (int(iok) < 0)
        throw CanteraError("MultiNewton::step",
                           "iok = " + int2str(iok));
    ...
    if (!ok) {
        for (size_t n = 0; n < sz; n++) {
            ...
            int pt = (n - d->loc()) / nvd;
            cout << "step: " << pt << "  " << r.pointDomain(n)->componentName(n - d->loc() - nvd * pt)
                       ...
                       OneDim& r, MultiJac& jac, int loglevel)
{
    ...
    while (1 > 0) {
        ...
        if (loglevel == 1 && m >= 0) {
            if (frst) {
                sprintf(m_buf, "\n\n    %10s    %10s   %5s ", "log10(ss)", "log10(s1)", "N_jac");
                writelog(m_buf);
                sprintf(m_buf, "\n    ------------------------------------");
                writelog(m_buf);
            }
