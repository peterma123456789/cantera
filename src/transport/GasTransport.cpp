...
GasTransport::GasTransport(ThermoPhase* thermo) : Transport(thermo),
                                                  m_viscmix(0.0),
                                                  m_visc_ok(false),
                                                  m_viscwt_ok(false),
                                                  m_spvisc_ok(false),
...
GasTransport::GasTransport(const GasTransport& right) : m_viscmix(0.0),
                                                        m_visc_ok(false),
                                                        m_viscwt_ok(false),
                                                        m_spvisc_ok(false),
                                                        m_bindiff_ok(false),
    ...
    if (m_mode == CK_Mode) {
        for (size_t i = 0; i < m_nsp; i++) {
            for (size_t j = i; j < m_nsp; j++) {
                ...
                //printf("%10s-%10s:%16.5g%16.5g%16.5g%16.5g:%16.5g\n", m_thermo->speciesName(i).c_str(), m_thermo->speciesName(j).c_str(), m_diffcoeffs[ic][0], m_diffcoeffs[ic][1], m_diffcoeffs[ic][2], m_diffcoeffs[ic][3], m_bdiff(i, j));
                ic++;
            }
        }
    } else {
