...
WaterTransport::WaterTransport(thermo_t* thermo, int ndim) : Transport(thermo, ndim)
{
    initTP();
}

WaterTransport::WaterTransport(const WaterTransport& right) : Transport(right.m_thermo, right.m_nDim)
{
    *this = right;
}
